...
class Zephyr(commands.Cog):
    ...
    def get_from_roster(self):
        ...
        members = service.spreadsheets().values().get(spreadsheetId=spreadsheet_id, range=("'Member List'!B:G")).execute().get('values')
    ...
    async def zephyr(self, ctx: commands.Context):
        ...
        # Get all 244th SteamIDs
        members = self.get_from_roster()

        online_players = "\nActive 244th. (Note: Just because someone is listed here does not mean they are on their whitelist. They are just on Zephyr.)"
        ...
        # Check if there is at least 1 online 224th
        if members_community_ids:
            ...
            # If there are no online 224th
            if online_players == "\nActive 224th. (Note: Just because someone is listed here does not mean they are on their whitelist. They are just on Zephyr.)":
                online_players = "\nNo online 224th."
